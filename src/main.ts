...
export default class SlidesPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    // Replace <hr> with styled slide separators in reading view
    this.registerMarkdownPostProcessor((el, ctx) => {
      if (!ctx.frontmatter?.slides) return;

      el.querySelectorAll("hr").forEach((hr) => {
        const sep = document.createElement("div");
        sep.className = "sp-reading-separator";
        const line = document.createElement("div");
        line.className = "sp-reading-separator-line";
